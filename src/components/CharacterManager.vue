<template>
  ...
  <div class="flex flex-col items-center justify-center h-screen w-full bg-white text-gray-800 p-4 sm:p-6 md:p-8 overflow-y-auto">
    ...
    <div v-if="!loreLoaded" class="w-full max-w-lg bg-gray-100 p-6 rounded-lg shadow-lg border border-gray-200 text-center">
      <p class="text-lg text-gray-700">Loading lore...</p>
    </div>

    <div v-else-if="currentStep === 'chapter1' || currentStep === 'factionSelect'" class="w-full max-w-3xl bg-gray-50 p-6 rounded-xl shadow-2xl border border-gray-200">
      <h3 class="text-2xl font-serif text-black mb-4 border-b border-gray-300 pb-2">Chapter 1: The First Whisper</h3>
      <div v-html="lore.chapter1" class="text-lg text-gray-800 leading-relaxed mb-6 max-h-80 overflow-y-auto markdown-content"></div>
      
      <div class="space-y-4">
        <button @click="currentStep = 'factionSelect'" v-if="currentStep === 'chapter1'"
                class="w-full py-3 px-4 rounded-lg text-xl font-bold bg-black hover:bg-gray-800 text-white shadow-lg transition-all duration-200 ease-in-out">
        ...
        <div v-if="currentStep === 'factionSelect'" class="space-y-4 mt-6">
          <button @click="selectFaction('Lumen')"
                  :class="['w-full py-3 px-4 rounded-lg text-xl font-bold transition-all duration-200 ease-in-out',
                           newCharacter.faction === 'Lumen' ? 'bg-black text-white shadow-xl' : 'bg-gray-200 hover:bg-gray-300 text-black']">
            Select Lumen
          </button>
          <button @click="selectFaction('Umbra')"
                  :class="['w-full py-3 px-4 rounded-lg text-xl font-bold transition-all duration-200 ease-in-out',
                           newCharacter.faction === 'Umbra' ? 'bg-black text-white shadow-xl' : 'bg-gray-200 hover:bg-gray-300 text-black']">
            Select Umbra
          </button>
          <button @click="goToNextStep" :disabled="!newCharacter.faction"
                  class="w-full py-3 px-4 rounded-lg text-xl font-bold bg-black hover:bg-gray-800 text-white shadow-lg transition-all duration-200 ease-in-out disabled:opacity-50 disabled:cursor-not-allowed">
            Proceed
          </button>
    ...
    <div v-else-if="currentStep === 'chapter2' || currentStep === 'specialtySelect'" class="w-full max-w-3xl bg-gray-50 p-6 rounded-xl shadow-2xl border border-gray-200">
      <h3 class="text-2xl font-serif text-black mb-4 border-b border-gray-300 pb-2">Chapter 2: The Second Whisper</h3>
      <div v-html="lore.chapter2" class="text-lg text-gray-800 leading-relaxed mb-6 max-h-80 overflow-y-auto markdown-content"></div>
      ...
      <div class="space-y-4">
        ...
        <div v-if="currentStep === 'specialtySelect'" class="space-y-4 mt-6">
          <button @click="selectSpecialty('Arcane')"
                  :class="['w-full py-3 px-4 rounded-lg text-xl font-bold transition-all duration-200 ease-in-out',
                           newCharacter.specialty === 'Arcane' ? 'bg-black text-white shadow-xl' : 'bg-gray-200 hover:bg-gray-300 text-black']">
            Select Arcane
          </button>
          <button @click="selectSpecialty('Alchemist')"
                  :class="['w-full py-3 px-4 rounded-lg text-xl font-bold transition-all duration-200 ease-in-out',
                           newCharacter.specialty === 'Alchemist' ? 'bg-black text-white shadow-xl' : 'bg-gray-200 hover:bg-gray-300 text-black']">
            Select Alchemist
          </button>
    ...
    <div v-else-if="currentStep === 'nameAssign'" class="w-full max-w-3xl bg-gray-50 p-6 rounded-xl shadow-2xl border border-gray-200">
      <h3 class="text-2xl font-serif text-black mb-4 border-b border-gray-300 pb-2">Assign Your Scholar Name</h3>
      ...
      <p class="text-sm text-gray-600 mb-4 italic">
        Current Location: 
        <span v-if="location.loading" class="text-gray-500">Detecting location...</span>
        <span v-else-if="location.name">{{ location.name }}</span>
        <span v-else class="text-red-500">Location Unknown (This will be 'Unknown' on the global leaderboards)</span>
      </p>

      <form @submit.prevent="createCharacter" class="space-y-4">
        <div>
          <label for="characterName" class="block text-lg text-gray-700 mb-2">Scholar's Name:</label>
          <input
            id="characterName"
            v-model="newCharacter.name"
            type="text"
            required
            class="w-full p-3 rounded-lg bg-white text-black border border-gray-300 focus:outline-none focus:ring-2 focus:ring-black"
...
import { getFirestore, collection, addDoc, doc, updateDoc, increment, setDoc, getDoc } from 'firebase/firestore'; // Added getDoc
...
// --- UPDATED MARKDOWN PARSER FUNCTION (Styled for Light Theme) ---
/**
...
<style>
/* Global styles for markdown content (can be moved to style.css if preferred) */
/* These are light-theme specific styles for the markdown parser */
.markdown-content h2 {
    color: #000;
